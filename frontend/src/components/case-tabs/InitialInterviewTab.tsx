    ...
    Promise.all([
      ...
      .then(([caseData, interview]) => {
        ...
        setForm({
          ...
          gender:           interview?.gender           || "Male",
          dob:              interview?.dob              || (caseData?.client?.dateOfBirth ? caseData.client.dateOfBirth.split("T")[0] : ""),
          firstName:        interview?.firstName        || caseData?.client?.firstName   || "",
          lastName:         interview?.lastName         || caseData?.client?.lastName    || "",
          unitNumber:       interview?.unitNumber       || "",
          streetNumber:     interview?.streetNumber     || "",
          streetName:       interview?.streetName       || caseData?.clientStreet        || "",
          city:             interview?.city             || caseData?.clientCity          || "",
          province:         interview?.province         || caseData?.clientState         || "Ontario",
          postalCode:       interview?.postalCode       || caseData?.clientZip           || "",
          country:          interview?.country          || caseData?.clientCountry       || "Canada",
          homePhone:        interview?.homePhone        || caseData?.client?.homePhone   || "",
          mobile:           interview?.mobile           || caseData?.clientMobile        || "",
          otherPhone:       interview?.otherPhone       || caseData?.client?.workPhone   || "",
          whatsapp:         interview?.whatsapp         || "",
          email:            interview?.email            || caseData?.client?.email       || "",
          maritalStatus:    interview?.maritalStatus    || caseData?.client?.maritalStatus || "Married",
          dependants:       interview?.dependants       || String(caseData?.client?.dependants ?? "0"),
          ...
          languageNeeds:    interview?.languageNeeds    || "",
          electronicConsent: (interview?.electronicConsent || "Yes") as YN,
          benefitChosen:    interview?.benefitChosen    || "Income Replacement Benefit",
          // ID Docs — from interview GET response (loaded from case_client_id_docs)
          driverLicense:    interview?.driverLicense    || "",
          driverLicenseCopy: (interview?.driverLicenseCopy || "No") as YN,
          healthCard:       interview?.healthCard       || "",
          healthCardCopy:   (interview?.healthCardCopy  || "No") as YN,
          sin:              interview?.sin              || "",
          sinCopy:          (interview?.sinCopy         || "No") as YN,
          ontarioId:        !!(interview?.ontarioId),
          ontarioIdNo:      interview?.ontarioIdNo      || "",
          ontarioIdCopy:    (interview?.ontarioIdCopy   || "No") as YN,
          prCard:           !!(interview?.prCard),
          prCardNo:         interview?.prCardNo         || "",
          prCardCopy:       (interview?.prCardCopy      || "No") as YN,
          citizenCard:      !!(interview?.citizenCard),
          citizenCardNo:    interview?.citizenCardNo    || "",
          citizenCardCopy:  (interview?.citizenCardCopy || "No") as YN,
        });
      })
